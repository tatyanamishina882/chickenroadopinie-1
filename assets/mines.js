...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a4cd830c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a4cd830c">'+'<span class="tile-face-a4cd830c tile-front-a4cd830c"></span>'+'<span class="tile-face-a4cd830c tile-back-a4cd830c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a4cd830c")){return;}
var back=tile.querySelector(".tile-back-a4cd830c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a4cd830c");if(isMine){tile.classList.add("boom-a4cd830c");}
if(dim){tile.classList.add("dimmed-a4cd830c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a4cd830c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a4cd830c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a4cd830c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a4cd830c");void panelEl.offsetWidth;panelEl.classList.add("jolt-a4cd830c");}
endRound();}
